...
import { getTraditions, REGIONS } from '@/lib/api';
import { Tradition, Pagination as PaginationType } from '@/types';
import ItemCard from '@/components/ui/ItemCard';
import EmptyState from '@/components/ui/EmptyState';
import LoadingState from '@/components/ui/LoadingState';

export default function TraditionsPage() {
  const [items, setItems] = useState<Tradition[]>([]);
      ...
      <div className="gradient-primary py-12">
        <div className="container-custom">
          ...
          <p className="text-primary-100">
            ประเพณีไทยที่สืบทอดมา อนุรักษ์วัฒนธรรมและส่งเสริมความเข้มแข็งของชุมชน
          </p>
        </div>
      </div>
          ...
          <>
            {layout === 'grid' ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {items.map((item) => (
                  <ItemCard key={item.id} item={item} type="tradition" layout="grid" />
                ))}
              </div>
            ) : (
              <div className="space-y-4">
                {items.map((item) => (
                  <ItemCard key={item.id} item={item} type="tradition" layout="list" />
                ))}
              </div>
            )}
