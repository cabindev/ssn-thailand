...
import { getCreativeActivities, REGIONS } from '@/lib/api';
import { CreativeActivity, Pagination as PaginationType } from '@/types';
import ItemCard from '@/components/ui/ItemCard';
import EmptyState from '@/components/ui/EmptyState';
import LoadingState from '@/components/ui/LoadingState';

export default function CreativeActivitiesPage() {
  const [items, setItems] = useState<CreativeActivity[]>([]);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await getCreativeActivities({
      ...
      <div className="gradient-primary py-12">
        <div className="container-custom">
          ...
          <p className="text-primary-100">
            รวบรวมกิจกรรมสร้างสรรค์ พื้นที่สร้างสรรค์
          </p>
        </div>
      </div>
          ...
          <>
            {layout === 'grid' ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {items.map((item) => (
                  <ItemCard key={item.id} item={item} type="creative-activity" layout="grid" />
                ))}
              </div>
            ) : (
              <div className="space-y-4">
                {items.map((item) => (
                  <ItemCard key={item.id} item={item} type="creative-activity" layout="list" />
                ))}
              </div>
            )}
