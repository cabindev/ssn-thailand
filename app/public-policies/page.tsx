...
import { getPublicPolicies, REGIONS, POLICY_LEVELS } from '@/lib/api';
import { PublicPolicy, Pagination as PaginationType } from '@/types';
      ...
      <div className="gradient-primary py-12">
        <div className="container-custom">
          ...
          <p className="text-primary-100">
            นโยบายและมาตรการระดับต่างๆ ที่สนับสนุนการพัฒนาสังคมและชุมชนอย่างยั่งยืน
          </p>
        </div>
      </div>
          ...
          <>
            {layout === 'grid' ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {items.map((item) => (
                  <ItemCard key={item.id} item={item} type="public-policy" layout="grid" />
                ))}
              </div>
            ) : (
              <div className="space-y-4">
                {items.map((item) => (
                  <ItemCard key={item.id} item={item} type="public-policy" layout="list" />
                ))}
              </div>
            )}
